...
export class CryptoPriceChartComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.updateChart();
    //Subscribe to data updates from the CryptoPriceService
  ...
  // Copy and paste it into CryptoPriceChartComponent
  updateChart(): void {
    ...
    this.cryptoPriceService.getCryptoPriceData().subscribe((data: any) => {
      ...
      // Create a new Chart.js line charts
      this.chart = new Chart("cryptoChart", {
        ...
        options: {
          responsive: true,
          // Make the chart responsive to the container's size
          scales: {
            y: {
              beginAtZero: false,
              // Do not force the y-axis to start at zero (allow dynamic range)
            },
          },
        },
      });
